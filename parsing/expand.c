...
/* !!      there is a big issue should


// if there is a qoutation should not split
// if there is no qutation should split

$a => split if there is  space
"$a" => no split even if there is a space
...
void ft_expand(t_token *tokens)
{
	...
	while (tokens)
	{
		...
		{
			expanded = expand_string(tokens->value, &flag); //  expand_string(tokens->value, &falg);
			free(tokens->value);
